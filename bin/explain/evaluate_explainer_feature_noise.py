...
def filter_useless_features_multiclass(args_hops=args.hops,
    ...
    for i in dataset:
        ...
        print('i:',i)'''
        args_num_noise_feat = int(args_prop_noise_feat * i.x.size(1))
        if not node_indices:
        ...
        data, noise_feat = add_noise_features(i, args_num_noise_feat, binary=False, p=0.5)
    ...
    model = CustomGCN(
                type = args.model,
                num_node_features = 39, ####### LOOOOOOOOK HEREEEEEEEEE
    ...
    if args.training is True:
        ...
        if args.save_model is True:
            ...
            torch.save(model.state_dict(), '../data/models/gat_noise_feature_model.pth')
        ...
        checkpoint = torch.load('../data/models/pna_noise_feature_model.pth')
    ...
    for i in range(len(test_dataset)):
        ...
        for node_idx in tqdm(node_indices, desc='explain feature', leave=False):
            ...
            num_noise_feats = []  # one element for each class of a test sample
            ...
            for i in range(len(test_graph.y)):
                ...
                # Store indexes of K most important features, for each class
                arr = np.array(coefs)
                feat_indices = np.abs(arr[:]).argsort()[-args_K:].tolist()
                ...
                if args.explainer_name == 'GNNExplainer':
                    ...
                    num_noise_feat = sum(
                        len(idx) < num_noise_feat_considered for idx in feat_indices)
                elif args.explainer_name == 'LIME': 
                    num_noise_feat = sum(
                        idx < num_noise_feat_considered for idx in feat_indices)
                num_noise_feats.append(num_noise_feat)
        ...
        if info:
            ...
            print('Proportion of explanations showing noisy neighbours: {:.2f}%'.format(100 * sum(total_num_noise_feats) / (args_K * args_test_samples * test_graph.y.item())))

            perc = 100 * sum(total_num_noise_feats) / \
                 (args_K * args_test_samples * test_graph.y)

            print(
                'Proportion of explanations showing noisy features: {:.2f}%'.format(perc.item()))

            
            if sum(total_num_noise_feat_considered) != 0:
                perc = 100 * sum(total_num_noise_feats) / \
                    (sum(total_num_noise_feat_considered)*test_graph.y.item())
                perc2 = 100 * (args_K * args_test_samples * test_graph.y.item() - sum(total_num_noise_feats)) / (
                    test_graph.y.item() * (sum(F) - sum(total_num_noise_feat_considered)))
...
def filter_useless_nodes_multiclass(args_hops=args.hops,
    ...
    for i in range(len(test_dataset)):
        ...
        for node_idx in tqdm(node_indices, desc='explain node', leave=False):
            ...
            #    print('n_idx:',type(n_idx.item()))
            #print('test_graph.x.size(0)-args_num_noise_nodes:',type(test_graph.x.size(0)-args_num_noise_nodes))    
            
            num_noisy_nodes = len(
                    [n_idx for n_idx in explainer.neighbours if n_idx.item() >= test_graph.x.size(0)-args_num_noise_nodes])
            ...
            for i in range(len(test_graph.y)):
                ...
                #print('(explainer.neighbours.shape[0] - num_noisy_nodes):',(explainer.neighbours.shape[0] - num_noisy_nodes))    
                ...
                if args.explainer_name == 'GNNExplainer':
                    ...
                    num_noise_nei = sum(
                        len(idx) >= (explainer.neighbours.shape[0] - num_noisy_nodes) for idx in nei_indices)
        ...
        if info:
            ...
            print('Proportion of explanations showing noisy neighbours: {:.2f}%'.format(100 * sum(total_num_noise_neis) / (args_K * args_test_samples * test_graph.y.item())))

            perc = 100 * sum(total_num_noise_neis) / (args_test_samples *
                                                        args_num_noise_nodes * test_graph.y)
            perc2 = 100 * ((args_K * args_test_samples * test_graph.y.item()) -
            ...
            print('Proportion of noisy neighbours in subgraph found in explanations: {:.2f}%'.format(
                    100 * sum(total_num_noise_neis) / (sum(total_num_noisy_nei) * test_graph.y.item())))
...
if args.noise == 'Node':
    ...
    filter_useless_features_multiclass(args_hops=args.hops,
                                        args_num_samples=args.num_samples,
                                        args_test_samples=args.test_samples,
