...
class MMAConv(MessagePassing):
    r"""
    The Multi-Mask Aggregation Convolution operator
    from the `"Multi-Mask Aggregators for GNNs"
    <https://openreview.net/forum?id=hZ3b8CskgC>`_ paper
    ...
    def __init__(self, in_channels: int, out_channels: int,
                 ...
                 divide_input: bool = False, device='cuda', **kwargs):
        ...
        for _ in range(towers):
            ...
            for aggr in aggregators:
                modules = [MaskAggregateLinear((3 if edge_dim else 2) * self.F_in, self.F_in, aggregators, aggr, mask=self.mask, device=self.device)]
                for _ in range(pre_layers - 1):
                    modules += [ReLU()]
                    modules += [MaskAggregateLinear(self.F_in, self.F_in, aggregators, aggr, mask=self.mask, device=self.device)]
