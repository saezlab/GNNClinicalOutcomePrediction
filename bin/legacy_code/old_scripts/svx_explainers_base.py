...
from svx_plots import (denoise_graph, k_hop_subgraph, log_graph,
...
class GraphSVX():
        ...
        for node_index in node_indexes:
            ...
            with torch.no_grad():
                ...
                true_conf, true_pred = self.model(
                        self.data.x.cuda(), 
                        self.data.edge_index.cuda(), batch = batch).exp()[node_index].max(dim=0)
    ...
    def explain_graphs(self,
        ...
        Returns:
                [tensors]: shapley values for features/neighbours that influence node v's pred
                        and base value
        """
        self.dat_ = self.data#[graph_indices]
        ...
        for graph_index in graph_indices:
            ...
            device = torch.device(
                        'cuda' if torch.cuda.is_available() else 'cpu')
            self.model = self.model.to(device)
            print('self.dat_.x:',self.dat_.x)
            batch = torch.zeros(self.dat_.x.shape[0], dtype=int, device=self.dat_.x.device).cuda()
            with torch.no_grad():
                        #print('self.model:', self.model.shape)
                        #print('self.data.x:', self.data.x.shape)
                        #print('self.data.edge_index:', self.data.edge_index.shape)
                true_conf, true_pred = self.model(self.dat_.x.cuda(),
                            self.dat_.edge_index.cuda(), batch = batch).exp()[graph_index,:].max(dim=0)
            ...
            #print('D:', D)
            '''print('self.dat_.edge_index.shape[1]:', self.dat_.edge_index.shape[1])
            print('self.dat_.edge_index[graph_indices]:', self.dat_.edge_index[graph_index].shape)
            print('np.diag(self.dat_.edge_index[graph_indices]):', np.diag(self.dat_.edge_index[graph_index]).shape)
            print('np.sum(np.diag(self.dat_.edge_index[graph_indices])):', np.sum(np.diag(self.dat_.edge_index[graph_index])))
            print('(self.dat_.edge_index.shape[1] - np.sum(np.diag(self.dat_.edge_index[graph_indices]))):', (self.dat_.edge_index.shape[1] - np.sum(np.diag(self.dat_.edge_index[graph_index]))))
            print('range(int(self.dat_.edge_index.shape[1] - np.sum(np.diag(self.dat_.edge_index[graph_indices])))):',range(int(self.dat_.edge_index.shape[1] - np.sum(np.diag(self.dat_.edge_index[graph_index])))))
            print('list(range(int(self.dat_.edge_index.shape[1] - np.sum(np.diag(self.dat_.edge_index[graph_indices]))))):', list(range(int(self.dat_.edge_index.shape[1] - np.sum(np.diag(self.dat_.edge_index[graph_index]))))))'''    
                # Total number of features + neighbours considered for node v
            self.neighbours = list(
                    range(int(self.dat_.edge_index.shape[1] - np.sum(np.diag(self.dat_.edge_index[graph_index])))))
            ...
            fz = self.graph_classification(
                    graph_indices, num_samples, D, z_, args_K, args_feat, true_pred)
    ...
    def compute_pred_subgraph(self, node_index, num_samples, D, z_, feat_idx, one_hop_neighbours, args_K, args_feat, discarded_feat_idx, multiclass, true_pred):
        ...
        for (key, ex_nei), (_, ex_feat) in tqdm(zip(excluded_nei.items(), excluded_feat.items())):
            ...
            with torch.no_grad():
                ...
                proba = self.model(X.cuda(), A.cuda(), batch=batch).exp()[node_index]
    ...
    def graph_classification(self, graph_index, num_samples, D, z_, args_K, args_feat, true_pred):
        ...
        for i in range(num_samples):
            ...
            for j in range(D):
                print('j:', j)
                print('D:', D)
                print('z_[i, self.F+j]:', z_[i, self.F+j])
                if z_[i, self.F+j] == 0:
                    nodes_id.append(self.neighbours[j])
            excluded_nei[i] = nodes_id
            #print('nodes_id:',nodes_id)
            #print('excluded_nei[i]:', excluded_nei[i])
            
            # Dico with key = num_sample id, value = excluded neighbour index

        # Init 
        fz = torch.zeros(num_samples)
        adj = deepcopy(self.dat_.edge_index[graph_index])
        if args_feat == 'Null':
            av_feat_values = torch.zeros(self.dat_.x[graph_index].shape[1])
        else: 
            av_feat_values = self.dat_.x.mean(dim=0).mean(dim=0)
        ...
        for (key, ex_nei) in tqdm(excluded_nei.items()):
            ...
            X = deepcopy(self.dat_.x[graph_index])

            for nei in ex_nei:
                X[nei] = av_feat_values
                print('X[nei]:', X[nei].shape)

            # Apply model on (X,A) as input.
            
            with torch.no_grad():
                batch = torch.zeros(X.shape[0], dtype=int, device=X.device).cuda()
                proba = self.model(X.unsqueeze(0).cuda(), A.unsqueeze(0).cuda(), batch = batch).exp()
    ...
    def neutral(self, node_index, num_samples, D, z_, feat_idx, one_hop_neighbours, args_K, args_feat, discarded_feat_idx, multiclass, true_pred):
        ...
        for (key, ex_nei), (_, ex_feat) in zip(excluded_nei.items(), excluded_feat.items()):
            ...
            with torch.no_grad():
                batch = torch.zeros(X.shape[0], dtype=int, device=X.device).cuda()
                proba = self.model(X.cuda(), self.data.edge_index.cuda(), batch=batch).exp()[
                        node_index]
